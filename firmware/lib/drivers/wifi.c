...
{
    if (data == NULL || received_len == NULL) {
        return WIFI_FAIL;
    }

    *received_len = 0;

    char *ipd = strstr((char *)wifi_dataBuffer, "+IPD,");
    if (ipd == NULL) {
        return WIFI_OK;
    }
    ...
    uint16_t len = (uint16_t)atoi(ipd + 5);
    uint8_t *payload_start = (uint8_t *)(colon + 1);
    uint16_t header_len = payload_start - wifi_dataBuffer;

    if (wifi_dataBufferIndex < header_len + len) {
        return WIFI_OK;
    }
